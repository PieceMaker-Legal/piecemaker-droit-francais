...
(function cloudCliLauncher() {
  ...
  var mockBridge = {
    ...
    openLocal: function () {
      ...
      mockState.activeTarget = { kind: 'local', name: 'Local ' + PRODUCT_NAME, url: mockState.localWebUrl };
  ...
  CC.act = function (name, node) {
    switch (name) {
      case 'local':
        return CC.run('Starting Local ' + PRODUCT_NAME + '...', function () { return bridge.openLocal(); });
      case 'connect':
      ...
      case 'dashboard':
        return CC.run('Opening ' + PRODUCT_NAME + ' dashboard...', function () { return bridge.openCloudDashboard(); });
  ...
  CC.titlebar = function (state) {
    ...
    return '<div class="titlebar">' +
      '<div class="brand"><span>' + esc(state.appName || PRODUCT_NAME) + '</span></div>' +
      '<div class="tb-tabs no-drag">' + renderTabs(state) + '</div>' +
      '<span style="flex:1"></span>' +
      refreshAction +
      envActions +
      '<button class="btn sm tb-action no-drag" data-cc-action="connect" title="' + esc(authState(state) === 'expired' ? 'Reconnect your ' + PRODUCT_NAME + ' account' : accountLabel(state)) + '"><span class="dot" style="background:' + (conn ? 'var(--ok)' : (authState(state) === 'expired' ? 'var(--warn)' : 'var(--tx3)')) + '"></span>' + esc(accountLabel(state)) + '</button>' +
      logoutAction +
  ...
  CC.buildLocalServerSection = function (state, options) {
    ...
    if (options.includePreferences) {
      body +=
        '<label class="cc-toggle"><input type="checkbox" data-cc-setting="keepLocalServerRunning"' + (settings.keepLocalServerRunning ? ' checked' : '') + '><span><b>Keep server running</b><br>Leave Local ' + PRODUCT_NAME + ' available after you quit the app.</span></label>' +
    ...
    return CC.renderSection(
      ...
      options.title || 'Run Local ' + PRODUCT_NAME + ' on this machine',
      body
    );
  };
  ...
  CC.renderLocalSettings = function () {
    ...
    var sections = [
      ...
      CC.renderSection('PREFERENCES', 'How the local service behaves', '' +
        ...
        '<label class="cc-toggle"><input type="checkbox" data-cc-setting="keepLocalServerRunning"' + ((state.desktopSettings || {}).keepLocalServerRunning ? ' checked' : '') + '><span><b>Keep server running</b><br>Leave Local ' + PRODUCT_NAME + ' available after you quit the app.</span></label>' +
    ...
    CC.renderSheet('Local Settings', 'Manage how Local ' + PRODUCT_NAME + ' runs on this computer.', sections);
  };
...
(function sidebarApp() {
  ...
  function localPane(state) {
    return '<div class="pane-h"><div><h2 class="pane-title">Local servers</h2><p class="pane-sub">Manage Local ' + PRODUCT_NAME + ' on this machine. No account required.</p></div></div>' +
      ...
      '<div class="card-actions"><button class="btn pri" data-cc-action="local">' + CC.icon('play', 15) + 'Open Local ' + PRODUCT_NAME + '</button><button class="btn" data-cc-action="open-web">' + CC.icon('arrow', 14) + 'Open in browser</button><button class="btn" data-cc-action="copy-web">' + CC.icon('copy', 14) + 'Copy URL</button></div></div>';
  }
  ...
  function cloudPane(state) {
    ...
    if (CC.authState(state) === 'expired') {
      return header + '<div class="empty">Your ' + PRODUCT_NAME + ' session expired.<div style="margin-top:14px"><button class="btn pri" data-cc-action="connect">' + CC.icon('cloudPlus', 15) + 'Reconnect account</button></div></div>';
    }
    if (!CC.connected(state)) {
      return header + '<div class="empty">Connect your ' + PRODUCT_NAME + ' account to list hosted environments.<div style="margin-top:14px"><button class="btn pri" data-cc-action="connect">' + CC.icon('cloudPlus', 15) + 'Connect account</button></div></div>';
    }
    if (state.cloudLoading && !(state.environments || []).length) {
      return header + '<div class="empty">Loading your ' + PRODUCT_NAME + ' environments...</div>';
    }
