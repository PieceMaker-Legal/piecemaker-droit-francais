...
const { getAddonsData, downloadAddonsDocument } = vi.hoisted(() => ({
  getAddonsData: vi.fn(),
  downloadAddonsDocument: vi.fn(),
}));
vi.mock('@/piecemaker/addons/api', () => ({ getAddonsData, downloadAddonsDocument }));
...
beforeEach(() => {
  ...
  downloadAddonsDocument.mockReset();
});
...
describe('page Library', () => {
  it('affiche les dossiers et documents avec leurs métadonnées', async () => {
    ...
    expect(getAddonsData).toHaveBeenCalledWith('/library/file');
  });

  it('affiche une erreur récupérable', async () => {
    getAddonsData.mockReset().mockRejectedValueOnce(new Error('Bibliothèque Addons indisponible')).mockResolvedValueOnce(ROOT_RESPONSE);
    render(<LibraryPage />);
    expect((await screen.findByRole('alert')).textContent).toContain('Bibliothèque Addons indisponible');
  ...
  it('change la requête en descendant dans un dossier', async () => {
    ...
    await waitFor(() => expect(getAddonsData).toHaveBeenCalledWith('/library/file?parent_folder_id=folder-1'));
