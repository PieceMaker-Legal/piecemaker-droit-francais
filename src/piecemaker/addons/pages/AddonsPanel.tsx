...
import type { AddonsWorkflow, WorkflowAddon, WorkflowAddonDetail } from '@/piecemaker/addons/types';
...
type AddonPack = { pack_key: string; pack_title: string; pack_description: string | null; addons: WorkflowAddon[] };

export function AddonsPanel({ projectPath, search }: { projectPath: string | null; search: string }) {
  const { t } = useTranslation('addons');
  const { data, error, loading, reload } = useAddonsData<WorkflowAddon[]>('/workflow-addons');
  ...
  const addonTypeLabel = (type: WorkflowAddon['type']): string => type === 'assistant' ? t('common.typeAssistant') : t('common.typeTabular');
  ...
  const detail = useAddonsData<WorkflowAddonDetail>(selectedId ? `/workflow-addons/${selectedId}` : null);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-border/40 px-4 py-3">
        <PillBar>
          <Pill isActive={sourceFilter === 'all'} onClick={() => setSourceFilter('all')}>{t('addonsPanel.allSources')}</Pill>
          <Pill isActive={sourceFilter === 'mike'} onClick={() => setSourceFilter('mike')}>{t('addonsPanel.sourcePieceMaker')}</Pill>
          <Pill isActive={sourceFilter === 'claude-for-legal-fr'} onClick={() => setSourceFilter('claude-for-legal-fr')}>{t('addonsPanel.sourceClaudeForLegalFr')}</Pill>
        </PillBar>
      </div>
      <AddonsAsyncState
        loading={loading}
        loadingLabel={t('addonsPanel.loading')}
        error={error}
        onRetry={reload}
        empty={isEmpty}
        emptyLabel={addons.length === 0 ? t('addonsPanel.noAddons') : t('addonsPanel.noAddonsMatch')}
      />
              ...
              <>
                <div className="flex shrink-0 items-start justify-between gap-3 border-b border-border/40 px-4 py-3">
                  <div>
                    ...
                    {selected.assets.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-medium text-muted-foreground">{t('addonsPanel.resources')}</p>
                  ...
                  {projectPath && selected.type === 'assistant' && (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        appendAddonsWorkflowDraft(workflow);
                        setAddonsPage(null);
                      }}
                    >
                ...
                <ScrollArea className="min-h-0 flex-1 px-4 py-3">
                  {selected.prompt_md ? (
                    ...
                    <p className="text-sm text-muted-foreground">{t('addonsPanel.noInstructions')}</p>
                  )}
                </ScrollArea>
              </>
            )}
