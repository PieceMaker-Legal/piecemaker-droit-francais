...
beforeEach(() => {
  getAddonsData.mockReset().mockResolvedValue(WORKFLOWS);
});
...
describe('WorkflowPickerDialog', () => {
  ...
  it('affiche une erreur récupérable', async () => {
    getAddonsData.mockReset().mockRejectedValueOnce(new Error('Workflows Addons indisponibles')).mockResolvedValueOnce(WORKFLOWS);
    render(<WorkflowPickerDialog open onClose={vi.fn()} onSelect={vi.fn()} />);
    expect((await screen.findByRole('alert')).textContent).toContain('Workflows Addons indisponibles');
