...
export const PIECEMAKER_I18N_ADDITIONS: AdditionBundles = {
  fr:      { common: { tabs: { dossier: 'Dossier', library: 'Bibliothèque' } }, chat: { composer: { removeCommandPill: 'Retirer la commande {{name}}' } }, sidebar: { tooltips: { anonymizationComplete: 'Anonymisation effectuée' } }, settings: { about: { uninstall: 'Désinstaller PieceMaker', uninstallConfirm: 'Confirmer la désinstallation', uninstallCancel: 'Annuler', uninstallHint: 'Retire l’application, les certificats locaux et les composants Python. Vos dossiers restent sur cet ordinateur.', uninstallFailed: 'La désinstallation n’a pas pu démarrer.' } } },
  en:      { common: { tabs: { dossier: 'Case file', library: 'Library' } }, chat: { composer: { removeCommandPill: 'Remove the {{name}} command' } }, sidebar: { tooltips: { anonymizationComplete: 'Anonymization complete' } }, settings: { about: { uninstall: 'Uninstall PieceMaker', uninstallConfirm: 'Confirm uninstall', uninstallCancel: 'Cancel', uninstallHint: 'Removes the application, the local certificates and the Python components. Your case files stay on this computer.', uninstallFailed: 'Uninstall could not start.' } } },
