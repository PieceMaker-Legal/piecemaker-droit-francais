...
describe('page Revues tabulaires', () => {
  ...
  it('affiche une erreur récupérable', async () => {
    getMikeData.mockReset().mockRejectedValueOnce(new Error('Revues Mike indisponibles')).mockResolvedValueOnce(LIST_RESPONSE);
    render(<TabularReviewsPage />);
    expect((await screen.findByRole('alert')).textContent).toContain('Revues Mike indisponibles');
