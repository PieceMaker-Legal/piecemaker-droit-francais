...
import type { MikeTabularCellFlag, MikeTabularCellStatus, MikeTabularReview, MikeTabularReviewDetail } from '@/piecemaker/mike/types';
import { useMikeData } from '@/piecemaker/mike/useMikeData';

const dateFormatter = new Intl.DateTimeFormat('fr-FR');

const FLAG_DOT_CLASS: Record<MikeTabularCellFlag, string> = {
  green: 'bg-green-500',
...
const FLAG_LABEL: Record<MikeTabularCellFlag, string> = {
  green: 'Conforme',
  grey: 'Neutre',
  yellow: 'À vérifier',
  red: 'Problème',
...
const STATUS_LABEL: Record<MikeTabularCellStatus, string> = {
  pending: 'En attente',
...
function reviewTitle(review: MikeTabularReview): string {
  return review.title?.trim() || 'Revue sans titre';
}

export function TabularReviewsPage() {
  const [selectedId, setSelectedId] = useState<string | null>(null);

  const list = useMikeData<MikeTabularReview[]>('/tabular-review');
  const detail = useMikeData<MikeTabularReviewDetail>(selectedId ? `/tabular-review/${selectedId}` : null);

  const cellByKey = useMemo(() => {
    const lookup = new Map<string, MikeTabularReviewDetail['cells'][number]>();
    ...
    return (
      <div className="flex h-full min-h-0 flex-col">
        <MikeAsyncState
          ...
          loadingLabel="Chargement des revues tabulaires…"
          error={list.error}
          onRetry={list.reload}
          empty={isEmpty}
          emptyLabel="Aucune revue tabulaire disponible."
        />
            ...
            {reviews.map((review) => (
              ...
              >
                <div className="flex min-w-0 flex-1 items-center gap-2">
                  ...
                  {review.is_running && <Badge variant="secondary">En cours</Badge>}
                </div>
                <div className="flex shrink-0 items-center gap-3 text-xs text-muted-foreground">
                  <span>{review.document_count ?? 0} document{(review.document_count ?? 0) > 1 ? 's' : ''}</span>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <MikeAsyncState
        ...
        loadingLabel="Chargement de la revue…"
        error={detail.error}
        onRetry={detail.reload}
        empty={isEmpty}
        emptyLabel="Cette revue ne contient aucune ligne."
      />
      {!detail.loading && !detail.error && !isEmpty && (
        <div className="min-h-0 flex-1 overflow-x-auto overflow-y-auto">
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr>
                <th className="sticky left-0 top-0 z-20 border-b border-r border-border/40 bg-background px-3 py-2 text-left font-medium">Élément</th>
                {columns.map((column) => (
                  <th key={column.index} className="sticky top-0 z-10 min-w-48 border-b border-border/40 bg-background px-3 py-2 text-left font-medium">
                    {column.name || `Colonne ${column.index}`}
                  </th>
                ))}
              </tr>
            </thead>
