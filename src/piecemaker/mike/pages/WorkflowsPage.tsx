...
type WorkflowTypeFilter = 'all' | 'assistant' | 'tabular';
...
export function WorkflowsPage({ projectPath }: { projectPath: string | null }) {
  ...
  const filtered = useMemo(() => {
    ...
    return workflows.filter((workflow) => {
      if (typeFilter !== 'all' && workflow.metadata.type !== typeFilter) return false;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-border/40 px-4 py-3">
        ...
        <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Rechercher un workflow…" className="max-w-xs" />
      </div>
      <MikeAsyncState
        loading={loading}
              ...
              <>
                ...
                <ScrollArea className="min-h-0 flex-1 px-4 py-3">
                  <pre className="whitespace-pre-wrap break-words text-sm">
                    {selected.skill_md || 'Aucune instruction disponible pour ce workflow.'}
                  </pre>
                </ScrollArea>
              </>
            )}
