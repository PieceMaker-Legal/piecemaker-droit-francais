...
import { useMikeLibraryBrowser } from '@/piecemaker/mike/hooks/useMikeLibraryBrowser';
import { formatMikeDocumentSize } from '@/piecemaker/mike/libraryFormat';
...
export function DocumentPickerDialog({ open, onClose, onConfirm }: DocumentPickerDialogProps) {
  const {
    ...
    goToStackIndex,
    resetNavigation,
  } = useMikeLibraryBrowser('file', open);
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="flex max-h-[85dvh] w-[calc(100vw-2rem)] max-w-2xl flex-col overflow-hidden p-0">
        <DialogTitle>Choisir des documents</DialogTitle>
        <div className="flex shrink-0 items-center gap-3 border-b border-border/40 px-4 py-3">
          <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Rechercher un document…" className="max-w-xs" autoFocus />
        </div>
        {!isSearching && (
        ...
        <div className="flex shrink-0 items-center justify-between gap-2 border-t border-border/40 px-4 py-3">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={confirm} disabled={selectedIds.size === 0}>Confirmer</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
