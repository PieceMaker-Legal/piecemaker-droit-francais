...
def normalize_entity_text(text: str) -> str:
    ...
    several anonymisation codes. Measured on ALPHABIO_URD before normalisation:
...
def resolve_overlapping_spans(results: List[RecognizerResult]) -> List[RecognizerResult]:
    ...
    on ALPHABIO_URD, 81 spans carried two types at once and 179 pairs overlapped —
...
# Measured on 25 real contexts from ALPHABIO_URD, that call cost 2 890 ms each
...
def _is_plausible_ip(entity_text: str, text: str, start: int, end: int) -> bool:
    ...
    Measured on ALPHABIO_URD: 31 of 31 distinct IP_ADDRESS hits were heading numbers
